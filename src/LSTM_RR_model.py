...
def load_forcing(basin: str) -> Tuple[pd.DataFrame, int]:
    ...
    swe=loadDf('SWE_basin_79_23_forecast.csv')
    idx=swe.index[swe.index>='1979-01-02']
    PET=loadDf('PET_basin_79_23_forecast.csv').loc[idx]
    PP=loadDf('pp_basin_79_23_forecast.csv').loc[idx]
    Tmax=loadDf('tmaxERA5_forecast.csv').loc[idx]
...
def load_discharge(basin: str, area: int) ->  pd.Series:
    ...
    q.columns=['QObs(mm/d)']
    q=q.loc[q.index>=pd.to_datetime('1979-01-02')]
...
class CamelsTXT(Dataset):
    ...
    def _load_data(self):
        ...
        df.loc[dfQ.index,'QObs(mm/d)'] = dfQ.values
